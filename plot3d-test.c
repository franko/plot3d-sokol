...
void frame(const plot_geom *geom, int w, int h) {
    /* NOTE: the vs_params_t struct has been code-generated by the shader-code-gen */
    vs_params_t vs_params;
    hmm_mat4 proj = HMM_Perspective(60.0f, (float)w / (float)h, 0.01f, 10.0f);

    const float cos_theta = cos(state.eye_theta), sin_theta = sin(state.eye_theta);
    const float eye_distance = state.eye_radius;
    const float x_eye = cos_theta * cos(state.eye_phi) * eye_distance;
    const float y_eye = cos_theta * sin(state.eye_phi) * eye_distance;
    const float z_eye = sin_theta * eye_distance;
    hmm_mat4 view = HMM_LookAt(HMM_Vec3(x_eye, y_eye, z_eye), HMM_Vec3(0.0f, 0.0f, 0.0f), HMM_Vec3(0.0f, 0.0f, 1.0f));
    hmm_mat4 view_proj = HMM_MultiplyMat4(proj, view);
    hmm_mat4 scale = HMM_Scale(HMM_Vec3(0.2f, 0.2f, 1.0f));
    hmm_mat4 model = scale;
...
int main(int argc, char **argv) {
    ...
    const int frame_duration = 1000 / 60; /* In milliseconds */
    int mouse_down = 0;
    Sint32 last_x, last_y;
    state.eye_theta = 3.14159265359f / 6;
    state.eye_phi = 0;
    state.eye_radius = 6.0f;
    const Uint64 perf_freq = SDL_GetPerformanceFrequency();
    while(!SDL_QuitRequested()){
        int w, h;
        SDL_GL_GetDrawableSize(state.window, &w, &h);
        frame(&state.geom, w, h);
        SDL_GL_SwapWindow(state.window);
        const Uint64 t_begin = SDL_GetPerformanceCounter();
        int frame_remaining = frame_duration;
        while (frame_remaining > 0 && SDL_WaitEventTimeout(NULL, frame_remaining)) {
            SDL_Event e;
            while (SDL_PollEvent(&e)) {
                ...
                case SDL_MOUSEMOTION:
                    if (mouse_down) {
                        state.eye_theta += (e.motion.y - last_y) * 0.01;
                        state.eye_phi   -= (e.motion.x - last_x) * 0.01;
                        last_x = e.motion.x;
                        last_y = e.motion.y;
                    }
                    break;
                case SDL_MOUSEWHEEL:
                    state.eye_radius += e.wheel.y * 0.2;
                    break;
                case SDL_QUIT:
                    goto quit_app;
                default:
            ...
            /* Below we multiply by 1000 to obtain milliseconds. frame_remaining is in ms. */
            frame_remaining = frame_duration - (1000 * (t_now - t_begin)) / (float) perf_freq;
        }
    }
quit_app:
    sg_shutdown();
